...
target_loader = input(f"{Fore.LIGHTCYAN_EX}IP/URL ⟩•> {Fore.WHITE}")
port_loader = int(input(f"{Fore.LIGHTCYAN_EX}PORT ⟩•> {Fore.WHITE}"))
time_loader = time.time() + int(input(f"{Fore.LIGHTCYAN_EX}TIME ⟩•> {Fore.WHITE}"))
spam_loader = int(input(f"{Fore.LIGHTCYAN_EX}SPAM THREAD ⟩•> {Fore.WHITE}"))
create_thread = int(input(F"{Fore.LIGHTCYAN_EX}CREATE THREAD ⟩•> {Fore.WHITE}"))
...
methods_loader = input(F"{Fore.LIGHTCYAN_EX}HTTP_METHODS (EXAMPLE=GATEWAY)⟩•> {Fore.WHITE}").upper()
